...
try{for(const width of [390,768,1440]){const context=await browser.newContext({viewport:{width,height:1000},colorScheme:'light',reducedMotion:'reduce'});const page=await context.newPage();
for(const [path,ref] of routes){await page.goto(base+path);await page.evaluate(()=>document.fonts.ready);const measured=reference.measurements.find(m=>new URL(m.url).pathname===(ref==='/article'?'/news':ref)&&m.width===width);
